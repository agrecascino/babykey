...
export struct JumpLocation {
    uint64_t order;
    uint64_t row;
};

export struct TickReturn {
    int16_t *audio[2];
    uint64_t nsamples;
    ReturnAction action;
...
nosample:
	...
	usleep((state.SamplesPerTick()/44100.0)*1000*1000);
        if(!tick)
            std::cout << std::endl;
        return ret;
    }
